...
import { TYPE_PRIORITIES as TYPES } from "../packages/plugin-wikidata/lib/prop.js";

const SOURCE_QUERY = `PREFIX csl: <https://citationstyles.org/ontology/type/>
SELECT DISTINCT ?item ?type WHERE {
  {
    VALUES (?uri ?type) { ${Object.keys(TYPES)
			.map((type) => `(csl:${type} "${type}")`)
			.join(" ")} }
