...
describe("input", function () {
			...
			for (const name of Object.keys(inputData[type])) {
				...
				describe(name, function () {
					...
					it("parses data", function () {
						assert.deepStrictEqual(plugins.input.chain(input, { generateGraph: false }), expected);
					});
				});
			}
		});
	...
	describe("@else/url", function () {
		...
		it("fails to load data", function () {
			assert.throws(() => plugins.input.chain("https://example.org/"), /^TypeError: No synchronous parser found for @else\/url$/);
		});

		it("fails to load data", function () {
			assert.rejects(() => plugins.input.chainAsync("https://example.org/"), /^TypeError: No parser found for @else\/url$/);
		});
	});
});
...
const outputData = {
	data: {
		...
		"weird html": [[{ author: [], editor: {} }], '<div class="csl-bib-body">[<div class="csl-entry">{<ul style="list-style-type:none"><li>"author": [],</li><li>"editor": {}</li></ul>}</div>]</div>', { format: "html" }],
	...
	ndjson: {
		...
		"downgrade csl": [[{ type: "software", version: "1.0.2", title: "Citation Style Language", "event-title": "RFC" }], JSON.stringify({ type: "book", version: "1.0.2", title: "Citation Style Language", event: "RFC" }), { version: "1.0.1" }],
		"upgraded csl": [[{ type: "software", version: "1.0.2", title: "Citation Style Language", "event-title": "RFC" }], JSON.stringify({ type: "software", version: "1.0.2", title: "Citation Style Language", "event-title": "RFC" })],
	},
	label: {
		...
		describe(type, function () {
			...
			for (const name of Object.keys(outputData[type])) {
				...
				it(`with ${name} works`, function () {
					...
					assert.deepStrictEqual(typeof actual === "string" ? actual.trim() : actual, expected);
				});
			}
		});
	}
